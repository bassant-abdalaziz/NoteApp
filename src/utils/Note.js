...
export function showAddModal({ token, updater }) {
  Swal.fire({
    ...
    // result of this function will be passed to allowOutsideClick after then
    preConfirm: () => {
      const title = document.getElementById("title").value;
      const content = document.getElementById("content").value;
      return { title, content }; // {title: title, content: content}
    },

    //result here object inside (title , content)
...
export async function getAllNotes({ token, updater }) {
  try {
    ...
    updater(data.notes); //setNotes(data.notes) fill array with data
  } catch (error) {
    updater([]);
  }
}
